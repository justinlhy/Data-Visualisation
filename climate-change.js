function ClimateChange() {
  ...
  this.draw = function () {
    ...
    for (var i = 0; i < this.data.getRowCount(); i++) {
      ...
      // Stop drawing this frame when the number of years drawn is equal to the frame count. This creates the animated effect
      // over successive frames.
      if (yearCount >= this.frameCount) {
        break;
      }

      // Assign current year to previous year so that it is available during the next iteration of this loop to give us the start
      // position of the next line segment.
      previous = current;
    }

    // Count the number of frames since this visualisation started. This is used in creating the animation effect and to
